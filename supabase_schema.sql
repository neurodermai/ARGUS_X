-- ═══════════════════════════════════════════════════════════════════════
-- ARGUS — Complete Supabase Schema v2.0
...
CREATE TABLE IF NOT EXISTS events (
  ...
  method               TEXT             DEFAULT '',
  -- ARGUS-specific fields (not in SENTINEL-LLM)
  sophistication_score INTEGER          DEFAULT 0,   -- 1-10 attacker intelligence
  attack_fingerprint   TEXT,                          -- Unique pattern family ID
  mutations_preblocked INTEGER          DEFAULT 0,   -- Variants auto-blocked after this
  session_threat_level TEXT             DEFAULT 'LOW' CHECK (session_threat_level IN ('LOW','MEDIUM','HIGH','CRITICAL')),
  explanation          TEXT                           -- XAI reasoning string
);
...
CREATE TABLE IF NOT EXISTS stats (
  ...
  bypasses_found       INTEGER          DEFAULT 0,   -- Red team discoveries
  mutations_preblocked INTEGER          DEFAULT 0,   -- Total variants auto-blocked
  campaigns_detected   INTEGER          DEFAULT 0,   -- Coordinated attacks detected
...
-- Logs autonomous red team agent activity
CREATE TABLE IF NOT EXISTS red_team_sessions (
  id            UUID             DEFAULT gen_random_uuid() PRIMARY KEY,
  created_at    TIMESTAMPTZ      DEFAULT NOW(),
  attack_text   TEXT,
  attack_type   TEXT,
  tier          INTEGER          DEFAULT 1,   -- 1=naive, 5=adversarial
...
-- Tracks unique attack pattern families and their recurrence
CREATE TABLE IF NOT EXISTS attack_fingerprints (
  id                UUID         DEFAULT gen_random_uuid() PRIMARY KEY,
  fingerprint_id    TEXT         UNIQUE NOT NULL,   -- e.g., "A1-ABC123DEF456"
  threat_type       TEXT,
...
-- ── CAMPAIGNS TABLE ───────────────────────────────────────────────────────
-- Coordinated attack campaigns detected by the Threat Correlator
CREATE TABLE IF NOT EXISTS campaigns (
  id               UUID         DEFAULT gen_random_uuid() PRIMARY KEY,
  campaign_id      TEXT         UNIQUE,
  type             TEXT,        -- RAPID | SUSTAINED | GLOBAL
  threat_type      TEXT,
...
-- ── DYNAMIC RULES TABLE ───────────────────────────────────────────────────
-- Rules added automatically by the mutation engine and red team agent
...
-- Shared threat intelligence (for federated network feature)
CREATE TABLE IF NOT EXISTS knowledge_base (
  ...
  pattern_vector  TEXT,        -- Embedding vector (stored as JSON)
  origin          TEXT         DEFAULT 'local',  -- local | federated
...
-- Public read (dashboard uses anon key to read)
...
-- ── ENABLE REALTIME ───────────────────────────────────────────────────────
-- IMPORTANT: Also enable in UI: Table Editor → each table → Realtime → ON
...
-- ── STORAGE ───────────────────────────────────────────────────────────────
-- Go to Storage → New Bucket → "argus-models" (Private)
-- Upload: sentinel_classifier.onnx + tokenizer files

-- ── VERIFICATION ──────────────────────────────────────────────────────────
DO $$
BEGIN
  RAISE NOTICE '✅ ARGUS Schema created successfully';
  RAISE NOTICE 'Tables: events, stats, red_team_sessions, attack_fingerprints, campaigns, dynamic_rules, knowledge_base';
  RAISE NOTICE 'Next: Enable Realtime on events + campaigns tables';
  RAISE NOTICE 'Next: Create Storage bucket "argus-models"';
END $$;
